...
pub use apply::{
    FLOAT_SCALE, enum_value_by_nick, float_from_api, float_to_api, set_property_from_api,
    set_property_from_value,
};
            ...
            .filter_map(|param_spec| {
                let id = match filter {
                    ...
                    ControlFilter::Encoder => {
                        resolve_pipeline_control_id(factory_name, param_spec.name(), &mut used_ids)
                    }
                };
                pspec::control_from_pspec(param_spec, factory_name, id)
            })
            .collect::<Vec<_>>();
...
pub fn list_encoder_controls(factory_name: &str) -> Vec<Control> {
    ...
    let controls = list_controls_on_element(&element, factory_name, ControlFilter::Encoder);
