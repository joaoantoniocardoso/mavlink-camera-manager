...
impl<'de> Deserialize<'de> for VideoCaptureConfiguration {
    fn deserialize<D: Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        let source_configuration = match raw.source_configuration {
            ...
            None => SourceConfiguration::ManualTranscoding(ManualTranscodingConfig {
...
mod tests {
    ...
    #[test]
    fn transcoding_json_without_source_configuration_infers_manual_x264enc() {
        let configuration: VideoCaptureConfiguration = serde_json::from_value(json!({
            "source_encode": "NV12",
            "encode": "H264",
            "width": 1280,
        ...
        assert_eq!(serialized["encode"], Value::String("H264".to_string()));
        assert!(serialized.get("source_configuration").is_some());
    }
