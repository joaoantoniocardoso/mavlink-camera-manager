...
pub struct WebRTCSink {
    ...
    first_frame_probe_id: Mutex<Option<gst::PadProbeId>>,
...
impl SinkInterface for WebRTCSink {
    ...
    fn link(
        ...
        let transceiver = {
            ...
            let probe_id = webrtcbin_sink_pad.add_probe(
            ...
            *self
                .first_frame_probe_id
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner()) = probe_id;
    ...
    fn shutdown_session(&self) {
        ...
        let leftover_block_probe_id = self
            .block_probe_id
            .lock()
            .unwrap_or_else(|poisoned| poisoned.into_inner())
            .take();
        if let Some(probe_id) = leftover_block_probe_id {
            let pad = self
                .block_pad
                .lock()
                .unwrap_or_else(|poisoned| poisoned.into_inner())
                .as_ref()
                .and_then(|weak_pad| weak_pad.upgrade());
            if let Some(pad) = pad {
                pad.remove_probe(probe_id);
            }
        }
...
impl WebRTCSink {
    ...
    pub fn try_new(
        ...
        let this = WebRTCSink {
            ...
            first_frame_probe_id: Mutex::new(None),
            tee_src_pad: None,
            bind,
            sender,
