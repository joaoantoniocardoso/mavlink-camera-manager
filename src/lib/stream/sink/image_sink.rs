...
impl ImageSink {
    ...
    async fn try_get_jpeg(&self, quality: u8, target_height: Option<u32>) -> Result<Vec<u8>> {
        ...
        // Flush the decoder to release its reference frame buffers.
        // Unlike Null cycling, flushing preserves sticky events on
        // pads so the next snapshot won't trigger "data flow before
        // stream-start" warnings or segfaults.  The pad blocker is
        // already re-installed by the appsink callback at this point.
        self.pipeline
            .send_event(gst::event::FlushStart::builder().build());
        self.pipeline
            .send_event(gst::event::FlushStop::builder(false).build());
