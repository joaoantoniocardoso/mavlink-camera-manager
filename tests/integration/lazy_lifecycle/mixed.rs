...
async fn test_webrtc_and_thumbnail_concurrent() {
    ...
    let webrtc = WebrtcClient::connect(&mcm.signalling_url(), None, None, None)
        .await
        .expect("negotiated WebRTC peer");
    webrtc
        .wait_for_frames(5, TIMEOUT)
        .await
        .expect("WebRTC media must flow before teardown");
    ...
    // Dropping the peer closes DTLS, which is the production mixed unlink:
    // WebRTC comes off the tee while the thumbnail ImageSink stays attached.
    drop(webrtc);
    ...
    client
        .delete_stream("mixed_conc")
        .await
        .expect("DELETE /streams after mixed WebRTC+thumbnail must not abort MCM");
    let streams = client.list_streams().await.unwrap();
    assert!(
        streams.is_empty(),
        "stream must be gone after DELETE; MCM still answering REST"
    );
}
