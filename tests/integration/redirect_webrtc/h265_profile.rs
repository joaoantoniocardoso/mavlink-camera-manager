...
///
/// Fails on current master (the H.265 branch strips `profile-id`,
/// `profile-space`, `tier-flag`, and friends, keeping only `sprop-*` and
/// forcing `level-id=93`). Passes once the rewrite stops discarding the
/// H.265 profile identity.
#[tokio::test]
