...
BINARY_NAME="mavlink-camera-manager-next"
...
sshpass -p "$REMOTE_PASS" ssh -o StrictHostKeyChecking=no \
    ...
    "docker exec ${CONTAINER} tmux send-keys -t video \
     '/root/${BINARY_NAME} --mavlink tcpout:127.0.0.1:5777 --mavlink-system-id 1 --mavlink-camera-component-id-range=100-105 --log-path /var/logs/blueos/services/mavlink-camera-manager --verbose' Enter"
